    ...
    <div ref={dropdownRef} className="relative">
      ...
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <Building2 size={20} className={`shrink-0 ${selectedBank ? 'text-accent-purple' : 'text-white/40'}`} />
          <span className={`truncate text-sm font-medium ${selectedBank ? 'text-white' : 'text-white/50'}`}>
            {loading ? 'Loading banks...' : selectedBank ? selectedBank.name : '-- Select your bank --'}
          </span>
        </div>
        <ChevronDown 
          size={18} 
              ...
              {filteredBanks.length === 0 ? (
                ...
                <div className="p-2">
                  {filteredBanks.map((bank) => (
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <Building2 size={18} className={`shrink-0 ${bank.code === value ? 'text-accent-purple' : 'text-white/40'}`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold truncate">{bank.name}</p>
                          <p className="text-xs text-white/50 mt-0.5">{bank.code}</p>
                        </div>
                      </div>
                      {bank.code === value && (
                        <Check size={18} className="text-accent-purple shrink-0" />
                      )}
                    </button>
                  ))}
                </div>
