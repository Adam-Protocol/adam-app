...
export default function SwapPage() {
  ...
  const estimatedOut = tokenIn === 'adusd'
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm">ADUSD ↔ ADNGN at live rates</p>
          </div>
        </div>
        ...
        {rate > 0 && (
          <div className="gradient-border rounded-xl p-4 mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="font-bold text-white text-sm">1 ADUSD = {rate.toFixed(2)} ADNGN</p>
            </div>
        ...
        <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-4">
          ...
          <div className="gradient-border rounded-2xl p-5 space-y-3">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 glass px-3 py-2.5 rounded-xl border border-white/10 min-w-[110px]">
                ...
                <span className="font-bold text-white text-sm uppercase">{tokenIn}</span>
              </div>
              <input
                type="number"
                step="0.01"
          ...
          {/* To */}
          <div className="gradient-border rounded-2xl p-5 space-y-3">
            <label className="block text-sm text-white/50">You Receive (est.)</label>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 glass px-3 py-2.5 rounded-xl border border-white/10 min-w-[110px]">
                <span>{tokenOut === 'adusd' ? '🇺🇸' : '🇳🇬'}</span>
                <span className="font-bold text-white text-sm uppercase">{tokenOut}</span>
              </div>
          ...
          <div className="flex items-start gap-3 glass px-4 py-3 rounded-xl border border-accent-cyan/20 text-sm">
            <Info size={16} className="text-accent-cyan mt-0.5 shrink-0" />
            <p className="text-white/50">Swap amounts are hidden on-chain. A new commitment is generated client-side for the output token.</p>
          </div>

          <button
            type="submit"
            disabled={!isConnected || mutation.isPending || !watch('amount_in')}
            className="btn-neon w-full py-4 rounded-2xl bg-gradient-to-r from-accent-cyan to-accent-purple text-white font-bold text-lg shadow-lg shadow-accent-cyan/30 disabled:opacity-50 transition-all"
          >
