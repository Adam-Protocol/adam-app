    ...
    items: [
      {
        ...
        inputs: [
          ...
          { name: 'token_to', type: 'core::starknet::contract_address::ContractAddress' },
        ],
...
export function useBuyRate(tokenOut: 'adusd' | 'adngn', amountIn: string) {
  ...
  useEffect(() => {
    ...
    const fetchRateAndFee = async () => {
      try {
        ...
        const rateResult = await contract.get_rate(CONTRACTS.USDC, tokenOutAddress);
        ...
        const feeResult = await contract.get_fee_bps();
  ...
  const effectiveRate = rate && feeBps !== undefined
